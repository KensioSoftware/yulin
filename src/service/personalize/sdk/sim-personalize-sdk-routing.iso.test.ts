...
describe("Personalize SDK interception", () => {
  ...
  it("routes every supported Command through the intercepted client", async () => {
    ...
    const schema = await client.send(
      new CreateSchemaCommand({ name: "interactions", schema: schemaDocument }),
    );
    ...
    await client.send(new DeleteSchemaCommand({ schemaArn: schema.schemaArn }));
    await client.send(
      new DeleteDatasetGroupCommand({ datasetGroupArn: group.datasetGroupArn }),
    );
