...
describe("Sim API Gateway REST API integration commands", () => {
  it("puts a Lambda proxy integration behind a method", async () => {
    ...
    // Then the method carries it, reporting the URI as it was configured, the
    // way real API Gateway echoes back the string it was given
    assertIdentical(integration.type, "AWS_PROXY");
    assertIdentical(integration.httpMethod, "POST");
    assertIdentical(integration.uri, invokePathUri);
    ...
    assertIdentical(method.methodIntegration.uri, invokePathUri);

    // And the function ARN the invoke path wrapped is held beside it, for
    // whatever has to reach the function
    const restApi = simAws.apiGateway().findRestApi(restApiId);
    assertIdentical(
      restApi?.requireResource(resourceId).findMethod("ANY")?.integration
        ?.lambdaUri.functionArn,
      functionArn,
    );
  });
