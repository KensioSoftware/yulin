...
- `ListSecrets` refuses `Filters` and `SortOrder` rather than ignoring them, since quietly returning an unfiltered list would be worse. Secrets are listed in creation order, with `MaxResults` and `NextToken` paging over them.
