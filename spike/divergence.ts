...
const probes: [string, string, string][] = [
  ...
  [
    ...
    `SELECT n FROM d.t ORDER BY n ASC`,
    "1, 3, NULL (nulls last)",
  ],
  [
    "null order ascending, rewritten",
    rewriteForSqlite(`SELECT n FROM d.t ORDER BY n ASC`),
    "1, 3, NULL (nulls last)",
  ],
  [
    "null order descending",
    `SELECT n FROM d.t ORDER BY n DESC`,
    "3, 1, NULL (nulls last)",
  ],
  ...
  [
    ...
    "query fails: Cannot cast",
  ],
  [
    "like case sensitivity",
    `SELECT count(*) AS v FROM d.t WHERE s LIKE 'alpha'`,
    "0 (LIKE is case sensitive)",
  ],
  [
    "concat a number",
    `SELECT 1 || 'x' AS v`,
    "query fails: no operator || for integer",
  ],
  ...
  [
    ...
    `SELECT length(s) AS v FROM d.t WHERE n = 3`,
    "NULL",
  ],
...
for (const [label, sql, athena] of probes) {
  ...
  console.log(
    `${label.padEnd(32)} sqlite ${sqlite.padEnd(46)} athena ${athena}`,
  );
}
